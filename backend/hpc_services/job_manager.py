...
HPC_BASE = os.getenv("HPC_PROJECT_BASE", "/gpfs/helios/home/hamza/projects/analytics-hub")
HPC_INPUT_BASE = f"{HPC_BASE}/input_data"
HPC_OUTPUT_BASE = f"{HPC_BASE}/output_data"
HPC_DRC_TIMEPOINT_SBATCH = os.getenv("HPC_DRC_TIMEPOINT_SBATCH", f"{HPC_BASE}/jobs/submit_drc.sh")
...
def submit_timepoint_job(project):
    ...
    hpc_config = {
        ...
        "top_n": project.config.get("top_n", 3),
    ...
    try:
        ...
        if not match:
            raise ValueError(f"Could not extract job ID from sbatch output: {sbatch_output}")
...
def check_job_status(job_id):
    ...
    try:
        ...
        if sacct_state == "COMPLETED":
            return "COMPLETED"
        elif sacct_state in ("FAILED", "CANCELLED", "TIMEOUT", "OUT_OF_MEMORY", "NODE_FAIL"):
            return "FAILED"
...
def delete_project_output_from_hpc(project_id):
    ...
    if not project_id or not project_id.startswith("prj_"):
        logger.warning(f"Skipping HPC cleanup — unexpected project_id format: {project_id}")
        return False
