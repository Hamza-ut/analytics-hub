...
def upload_file_to_hpc(local_file_path, upload_id):
    ...
    # 1. Fetch our base directory path from environment (or default to our known path)
    base_media_path = HPC_INPUT_DATA_BASE

    # Ensure base path finishes with a trailing slash for safety
    ...
    # 3. Perform the filesystem orchestration
    try:
        ...
        # Block script execution until cluster file system updates allocation
        ...
        # Crucial: Always drop network allocations when finished or when errors occur
        client.close()
...
def delete_file_from_hpc(remote_file_path):
    ...
    if not folder_name.startswith("upl_"):
        logger.warning(f"Skipping HPC deletion — folder '{folder_name}' lacks upl_ prefix.")
        return False

    if not remote_folder.startswith(expected_base):
        logger.critical(f"Skipping HPC deletion — {remote_folder} is outside {expected_base}.")
        return False
...
def download_file_from_hpc(remote_file_path):
    ...
    try:
        ...
        # 2. Open the file over SFTP in binary-read mode ('rb')
        remote_file = sftp.open(remote_file_path, "rb")

        # Attach the client handle directly to the file object wrapper
        # so it doesn't drop the connection until Django finishes streaming the data.
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n🗑️ STEP 2: Testing Isolated Deletion of that exact folder...")
