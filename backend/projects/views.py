...
def create_project(request):
    ...
    if request.method == "POST":
        ...
        pipeline_obj = get_object_or_404(
            Pipeline, pipeline_name=pipeline_name, is_active=True
        )
        ...
        if pipeline_name == "TIMEPOINT":
            ...
            config_data = {
                "group_fields": [
                    f.strip() for f in request.POST.get("group_fields", "").split(",")
                ],
            ...
            project = ProjectRun.objects.create(
                user=request.user, pipeline=pipeline_obj
            )
